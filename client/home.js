const socket = io.connect('https://shitchat.onrender.com')
const STORAGE_PREFIX = 'shit-';

window.onload = () => {
    const userId = sessionStorage.getItem(`${STORAGE_PREFIX}userStaticId`)
...
socket.on('message', (msg, q) => {
    ...
    style(socket.id, msg[1], msg_el, msgParent, name, q)
...
socket.on('show-static-id', id => document.querySelector('#static-id').innerText = id);
...
function style(socketId, userId, element, elementParent, elementName, query) {
    if (socketId == userId || (query && query !== 'other') || query === 'me') {
...
socket.on('msg-history-first', Data => {
